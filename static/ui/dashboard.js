...
async function loadAnaliseTemporal() {
    try {
        const res = await fetch('/api/dashboard/analise-temporal');
        const data = await res.json();
...
async function loadRankingDestinos() {
    try {
        const res = await fetch('/api/dashboard/ranking-destinos');
        const data = await res.json();
        ...
        container.innerHTML = data.slice(0, 10).map((d, i) => {
            const risco = parseFloat(d.risco_medio || 0);
            ...
            return `
                <div class="flex justify-between items-center py-1.5 border-b border-slate-50 last:border-0">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] font-medium text-slate-700 truncate max-w-[150px]" title="${d.destinos}">${d.destinos}</span>
                    </div>
                    <div class="flex items-center gap-2">
...
async function loadRankingOrgaos() {
    try {
        const res = await fetch('/api/dashboard/ranking-orgaos');
        const data = await res.json();
        ...
        container.innerHTML = data.slice(0, 10).map((d, i) => {
            ...
            return `
                <div class="flex justify-between items-center py-1.5 border-b border-slate-50 last:border-0">
                    <div class="flex items-center gap-2">
                        <span class="text-[9px] font-mono font-black text-slate-300">${i + 1}</span>
                        <span class="text-[10px] font-medium text-slate-700 truncate max-w-[150px]" title="${d.nome_do_orgao_superior}">${d.nome_do_orgao_superior}</span>
                    </div>
                    <div class="flex items-center gap-2">
...
async function loadTopAlvos() {
    try {
        ...
        container.innerHTML = data.slice(0, 10).map((d, i) => {
            ...
            const total = parseFloat(d.valor_diarias || 0) + parseFloat(d.valor_passagens || 0);
            ...
            return `
                <div class="flex flex-col py-2 border-b border-slate-50 last:border-0">
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-[9px] text-slate-400 truncate max-w-[150px]" title="${d.destinos}">${d.destinos}</span>
